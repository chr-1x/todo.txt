...
internal void
SerializeTodoItem(todo_item Item, uint32 ResultLength, char* Result)
{
...
SerializeTodoFile(todo_file Todo, bool32 RemoveEmptyLines)
{
    ...
    if (Todo.NumberOfItems > 0)
    {
        ...
        foreach(todo_item, It, Todo.NumberOfItems, Todo.Items)
        {
            ...
            Item = It;
			if (RemoveEmptyLines && !IsNonTrivial(Item->Body)) { continue; }
            uint32 ItemLength = GetItemStringSize(*Item);

            Assert((RunningSize + ItemLength) <= TotalSize);
            SerializeTodoItem(*Item, TotalSize - RunningSize, (char*)Result.Contents + RunningSize);
